<template>
  <div class="container">
    <h1>Intake CTF</h1>
    <div v-for="(challenges, category) in getChallengesByCategory()" :key="category" class="mb-4">
      <h3>{{ category }}</h3>
...
const getChallengesByCategory = () => {
  const categories: Record<string, Challenge[]> = {};
  challenges.value?.forEach((challenge) => {
    if (!(challenge.category in categories)) categories[challenge.category] = [];
